...
#    self.ctx.set_opt_string_list( "SRM PLUGIN", "TURL_PROTOCOLS", self.defaultLocalProtocols )
  ...
  def getOccupancy(self, *parms, **kws):    
    ...
    result = objectLoader.loadObject('Resources.Storage.occupancyPlugins.bdiiOccupancy')
    #    result = objectLoader.loadObject('Resources.Storage.occupancyPlugins.spaceTokenOccupancy')
    if not result['OK']:
      return result
    return result['Value']().getOccupancy(self)
